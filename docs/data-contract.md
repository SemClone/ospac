...
is only offered where it names every version it could be taken for: `php license v3` is a
choice between `PHP-3.0` and `PHP-3.01`, and `latex project public license v1` a choice
between five, so those are in `ambiguous` rather than `aliases`. Where it would name only
some of them it is absent entirely, which is why there is no
`gnu lesser general public license v2`: LGPL-2.0 ships under the name SPDX gave it,
"Library", so offering the two LGPL-2.1 readings would narrow a choice the string never
made.
