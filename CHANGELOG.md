...
**evaluate, check and obligations resolve the license string they are given** (#94)
- They matched identifiers exactly, so a string that was not already canonical SPDX
  reached no rule and came back `flag_for_review`, which a consumer cannot tell apart
  from a considered ruling. Registries do not answer in SPDX: PyPI's license field is
  free text by construction and requests 2.31.0 declares `Apache 2.0`. The data that
  resolves this has shipped since 1.6.0 and only the `aliases` command consulted it.
- `Apache 2.0` now approves where `Apache-2.0` approves, and `GNU General Public License
  v3.0 only` denies where `GPL-3.0-only` denies. The difference is load-bearing: a
...
- Rules see the caller's spelling as well as the resolved identifier, so a policy naming
  the deprecated `GPL-2.0` keeps matching and one naming `GPL-2.0-only` now matches the
  same input. A shipped identifier resolves to itself, so `GPL-2.0` still returns its own
  record and deprecation metadata.
- A string that names a license but not the grant is evaluated under every identifier it
  could mean, and asserts only what they agree on. The default policy denies both
  AGPL-3.0 grants for saas, so `evaluate -l "GNU Affero General Public License v3" -d
  saas` denies, and `check gplv2 BSD-4-Clause` reports the conflict BSD-4-Clause's record
  names against both GPL-2.0 grants.
- Where the readings disagree the answer is review, not the strictest of them: exactly
  one reading applies and nobody knows which. `cryptographic autonomy` is CAL-1.0 or its
  combined-work exception, network copyleft against permissive, so it reports review.
  Agreement is about the compliance outcome, so approve beside allow is two permissions
  and deny beside contaminate is two refusals.
- Only the obligations every reading carries are reported. `Apache License` is 1.0, 1.1
  or 2.0 and only 2.0 requires a NOTICE file. A dataset conflict holding under some
  readings and not all is review with a warning saying so.
- `obligations` answers for such a declaration instead of rejecting it.
  `ospac obligations -l "Apache License"` reported "Invalid license ID" for a string the
  shipped data recognises. `license_data` still holds records only, because a record
  merged from several readings would describe a license that does not exist; the
  candidates and their shared obligations go under `ambiguous_licenses`.
  `PolicyRuntime.get_obligations` resolves its argument too, and the `--policy-dir` path
  no longer exits 1 on an ambiguous declaration.
- `evaluate`, `check` and `obligations` report what they made of each declaration under
  `resolved_licenses`, so a caller can see that `Apache 2.0` was read as `Apache-2.0`
  rather than re-deriving the mapping to check.
- A license name containing a comma survives the CLI's comma-separated list. 31 names in
  the data contain one, and they are the prose spellings this release exists to serve.
  Fragments rejoin longest-first and no further than the longest name reaches.
- The shipped tables are read once per process rather than once per lookup. Splitting a
  hundred identifiers took five seconds and now takes three milliseconds.
- New public `ospac.resolve_license(text)` returning `LicenseResolution(text,
  license_id, candidates, status)`, `ospac.matchable_license_id(text)`,
...
**The alias data carries the version spelling registries write** (#95)
- SPDX names a license "GNU Affero General Public License v3.0" and Maven Central serves
  ...
  for the other, and the unknown one is the commoner form on Central. Seven GNU spellings
  were missing this way, not one.
- Derived rather than curated, over every spelling source. Each record claims its name
  with the minor dropped, and licenses that end up sharing a spelling collide into
  `ambiguous` with their candidates: `php license v3` is `PHP-3.0` or `PHP-3.01`, and
  `latex project public license v1` is one of five. AGPL-1.0's SPDX name omits "GNU", so
  running the derivation over the SPDX names alone answered for the v3 spelling and
  called the v1 one unknown.
- A shortened spelling is published only where its claimants cover every version at that
  major, compared on identifiers rather than on spellings. `apache v1` does not resolve
  to `Apache-1.1` with `Apache-1.0` beside it, and `gnu lesser general public license v2`
  is absent rather than offering the two LGPL-2.1 readings while LGPL-2.0 ships under the
  name SPDX gave it, "Library".
- 1739 aliases, up from 1669, and 143 ambiguous names, up from 113. Nothing was removed
  and nothing changed the identifier it resolved to.

### Added

- `tests/test_runtime.py::TestStrongerCopyleftIsNeverMorePermissive` pins the invariant
  behind #93: for every distribution type, `AGPL-3.0` may never evaluate more
  permissively than `GPL-3.0`. The gap itself was closed in #76 and nothing was holding
  it closed.
- Regenerating `aliases.json` from the shipped records is checked to reproduce the
  shipped file, rather than re-deriving the expected mapping by hand. The old shape would
  have let the generator and the data drift apart while still passing.
- Two properties the resolution path assumes are pinned: every lowercased identifier
  resolves, and `incompatible_with` names every spelling of a license that ships more
  than one.
